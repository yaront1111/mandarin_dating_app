// client/src/services/apiService.jsx
import axios from "axios";
import { toast } from "react-toastify";
import { getToken, setToken, removeToken, isTokenExpired } from "../utils/tokenStorage";

/**
 * Production-level Logger with configurable log levels.
 */
class Logger {
  constructor(name) {
    this.name = name;
...
/**
 * Production-level ResponseCache for caching API responses.
 */
class ResponseCache {
...
/**
 * Production-level NetworkMonitor for online/offline state.
 */
...
/**
 * Production-level RequestQueue to handle offline operations.
 */
class RequestQueue {
...
/**
 * Production-level API Service with advanced features.
 */
class ApiService {
  constructor() {
    ...
    try {
      ...
      if (idMatch && idMatch[1] && !url.includes("?") && !url.endsWith("/")) {
        ...
        if (potentialId.length === 24 && !this.isValidObjectId(potentialId)) {
          ...
          return { success: false, error: "Invalid ID format", status: 400 };
        }
      }
