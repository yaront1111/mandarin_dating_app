 ...
 * Verifies the JWT token from the socket handshake and attaches the user to the socket.
 *
 * @param {Object} socket - Socket.IO socket object.
 * @param {Function} next - Next function to call.
 * @param {Map<string, Set<string>>} userConnections - Map of user connections.
 */
const socketAuth = async (socket, next, userConnections) => {
  try {
    ...
    try {
      ...
      // Log the current state of userConnections for debugging
      logger.info(`Current user connections: ${Array.from(userConnections.keys()).length} users`)
      userConnections.forEach((sockets, uid) => {
        logger.info(`User ${uid}: ${sockets.size} active connections`)
      })

      next()
    } catch (error) {
      logger.warn(`Socket ${socket.id} connection rejected: ${error.message}`)
...
export { checkSocketPermission, setupSocketMonitoring, trackSocketActivity }
